...
public abstract class AbstractAgent<T extends WorldModel<? extends Entity>, E extends Entity> extends AbstractComponent<T> implements Agent {
	...
	@Override
	public final void postConnect(Connection c, EntityID agentID, Collection<Entity> entities, Config kernelConfig) throws Exception {
	...
	protected void processMessage(Message msg) {
		if (msg instanceof KASense) {
			...
			if (entityID.equals(sense.getAgentID())) {
				...
				System.out.println("sense of "+sense.getAgentID()+"recevied!");
			}
		} else {
	...
	private class AgentConnectionListener implements ConnectionListener {
		...
		/**
		 * @author Ali
		 *         if post connect has some error! we send the Acknowledge,too!!! To prevent running fail!
		 * @param c
		 * @param ok
		 */
		private void handleConnectOK(Connection c, KAConnectOK ok) {
			if (ok.getRequestID() == requestID) {
				...
				try {
					...
					Thread.currentThread().setName("[PreCompute]"+me().toString());
					try {
