...
public final class EncodingTools {
	...
	private static String COMPRESSED_MESSAGE="urn:rescuecore2:messages.compressed";
	public static boolean IS_COMPRESSION_ENABLED=false;
	...
	public static void writeMessage(Message m, OutputStream out) throws IOException {
		...
		if (IS_COMPRESSION_ENABLED) {
			compressMessage(m, out);
		} else {
			writeMessageNormally(m, out);
		}
		// Type URN
		// Size
		// Content
	...
	private static void writeMessageNormally(Message m, OutputStream out) throws IOException {
		// Type URN, size, content
		// Gather the content first
	...
	private static void compressMessage(Message m, OutputStream out) throws IOException {
		ByteArrayOutputStream bos = new ByteArrayOutputStream();
		writeMessageNormally(m, bos);
		ByteArrayOutputStream bos2 = new ByteArrayOutputStream();

		compress(new ByteArrayInputStream(bos.toByteArray()), bos2);

		writeString(COMPRESSED_MESSAGE, out);
		writeInt32(bos2.size(), out);
		out.write(bos2.toByteArray());

	}

	public static void compress(InputStream in, OutputStream outZip) throws IOException {
//		SevenZip.Compression.LZMA.Encoder encoder = new SevenZip.Compression.LZMA.Encoder();
//		encoder.SetEndMarkerMode(true);
//		encoder.WriteCoderProperties(outZip);
//
//		final long[] sizeTracker = new long[2];
//		encoder.Code(in, outZip, -1, -1, new ICodeProgress() {
//
//			@Override
//			public void SetProgress(long inSize, long outSize) {
//				sizeTracker[0] = inSize;
//				sizeTracker[1] = outSize;
//				System.out.println("in size"+inSize);
//			}
//		});
//		Logger.trace("insize:" + sizeTracker[0] + " outsize:" + sizeTracker[1] + " compress ratio:" + (100 - (sizeTracker[1] * 100) / sizeTracker[0] + "%"));
//		System.out.println(("insize:" + sizeTracker[0] + " outsize:" + sizeTracker[1] + " compress ratio:" + (100 - (sizeTracker[1] * 100) / sizeTracker[0] + "%")));

	}

	public static void decompress(InputStream inZip, OutputStream out) throws IOException {
//		SevenZip.Compression.LZMA.Decoder decoder = new SevenZip.Compression.LZMA.Decoder();
//		int propertiesSize = 5;
//		byte[] properties = new byte[propertiesSize];
//		if (inZip.read(properties, 0, propertiesSize) != propertiesSize)
//			throw new Error("input .lzma file is too short");
//		if (!decoder.SetDecoderProperties(properties))
//			throw new Error("Incorrect stream properties");
//		decoder.Code(inZip, out, -1);
	}
	...
	public static Message readMessage(InputStream in) throws IOException {
		...
		if (urn.equals(COMPRESSED_MESSAGE)) {
			ByteArrayInputStream bis = new ByteArrayInputStream(content);
			ByteArrayOutputStream bos = new ByteArrayOutputStream();
			decompress(bis, bos);
			return readMessage(new ByteArrayInputStream(bos.toByteArray()));
		}
