 ...
 */
public class SOSCommandLayer extends SOSAbstractSelectedLayer {
	...
	public Collection<RenderedObject> render(Graphics2D graphics, ScreenTransform transform, int width, int height) {
		synchronized (commands) {
			...
			for (Command next : commands) {
				if (next instanceof AKMove && (renderMove || (!renderMove && entities.contains(world.getEntity(next.getAgentID()))))) {
	...
	private Shape renderMove(AKMove move) {
		...
		g.draw(shape);
		Ellipse2D.Double shape2 = new Ellipse2D.Double(startX - SIZE / 2 / 4, startY - SIZE / 2 / 4, SIZE / 4, SIZE / 4);
		g.fill(shape2);
		return shape;
	}
