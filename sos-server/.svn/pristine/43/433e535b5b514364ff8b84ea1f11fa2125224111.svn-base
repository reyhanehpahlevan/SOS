...
public class CollapseSimulator extends StandardSimulator implements GUIComponent {
	...
	protected void postConnect() {
		...
		slight = new GaussianGenerator(config.getFloatValue(CONFIG_PREFIX + SLIGHT_MEAN_SUFFIX), config.getFloatValue(CONFIG_PREFIX + SLIGHT_SD_SUFFIX), config.getRandom());
		moderate = new GaussianGenerator(config.getFloatValue(CONFIG_PREFIX + MODERATE_MEAN_SUFFIX), config.getFloatValue(CONFIG_PREFIX + MODERATE_SD_SUFFIX), config.getRandom());
		severe = new GaussianGenerator(config.getFloatValue(CONFIG_PREFIX + SEVERE_MEAN_SUFFIX), config.getFloatValue(CONFIG_PREFIX + SEVERE_SD_SUFFIX), config.getRandom());
		destroyed = new GaussianGenerator(config.getFloatValue(CONFIG_PREFIX + DESTROYED_MEAN_SUFFIX), config.getFloatValue(CONFIG_PREFIX + DESTROYED_SD_SUFFIX),
				config.getRandom());
		block = config.getBooleanValue(BLOCK_KEY);
		floorHeight = config.getFloatValue(FLOOR_HEIGHT_KEY) * 1000;
		extent = new ContinuousUniformGenerator(config.getFloatValue(WALL_COLLAPSE_EXTENT_MIN_KEY), config.getFloatValue(WALL_COLLAPSE_EXTENT_MAX_KEY), config.getRandom());
	...
	protected void processCommands(KSCommands c, ChangeSet changes) {
		...
		System.out.println("Is aftershoke?"+model().aftershockHappens(time));
		Collection<Building> collapsed = doCollapse(changes, time);
		Map<Road, Collection<java.awt.geom.Area>> newBlock = doBlock(collapsed, time);
	...
	private Map<Road, Collection<java.awt.geom.Area>> doBlock(Collection<Building> collapsed, int time) {
	...
	private void createBlockages(Building b, Map<Road, Collection<java.awt.geom.Area>> roadBlockages, int time) {
		 ...
		 */
		if (model().getCSBuiding(b) == null)
			System.out.println(b);
		double remainingfloors = model().getCSBuiding(b).getRemainingToCollapse(floorHeight);
		 ...
		 */
		if (time != 1 && model().aftershockHappens(time) && !model().getCSBuiding(b).hasFireDamage()) {
			float intensity = model().aftershockIntensity(time);
			damage = (int) ((float) damage * Math.min(1, intensity));
		}
		/* Aftershock's Requirement: 2013 */
		double d = remainingfloors * (damage / (double) MAX_COLLAPSE) * currentExtent;
		t1 += System.currentTimeMillis() - t;
		t = System.currentTimeMillis();
		/* Aftershock's Requirement: 2013 */
		model().getCSBuiding(b).increaseCollapseRatio((float) (d / model().getCSBuiding(b).getTotalCollapse(floorHeight)));
