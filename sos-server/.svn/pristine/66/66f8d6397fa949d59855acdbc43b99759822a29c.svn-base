...
public abstract class AgentPath {
    ...
    public static AgentPath computePath(Human human, StandardWorldModel world) {
        ...
        AmbulanceTeam loaderAmbulance = getLoaderAmbulance(human,world);
        if (!human.isPositionHistoryDefined()&&loaderAmbulance==null
        		||(loaderAmbulance!=null&&!loaderAmbulance.isPositionHistoryDefined())) {
            // Agent didn't move.
            return null;
        }
        int[] history;
        if (human.isPositionHistoryDefined())
        		history = human.getPositionHistory();
        else if((loaderAmbulance!=null&&loaderAmbulance.isPositionHistoryDefined()))
        	history = loaderAmbulance.getPositionHistory();
        else
    ...
    private static AmbulanceTeam getLoaderAmbulance(Human human, StandardWorldModel world) {
    	if(world.getEntity(human.getPosition())instanceof AmbulanceTeam)
    			return ((AmbulanceTeam)world.getEntity(human.getPosition()));		
    	return null;
	}
