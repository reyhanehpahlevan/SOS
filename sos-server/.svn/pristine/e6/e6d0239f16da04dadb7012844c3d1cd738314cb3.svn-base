...
public class Building extends StationaryObject {
	...
	public double getRadiationEnergy() {
		double t = getTemperature() + 293; // Assume ambient temperature is 293
											// Kelvin.
	...
	public int getAverageCellTemprature(World w) {
		int sum=0;
		float c=0f;
		for (int[] nextCell : cells) {
			...
			double cellTemp = w.getAirCellTemp(cellX, cellY);
			sum+=cellTemp*cellCover;
			c+=cellCover;
		}
		return (int) (sum/c);
	}
