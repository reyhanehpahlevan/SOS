...
public class ShapeDebugFrame extends JFrame {
	public static final Icon TICK = new SOSImageIcon(Icons.class.getClassLoader().getResource("rescuecore2/view/tick.png"));

    /** A cross icon. */
    public static final Icon CROSS = new SOSImageIcon(Icons.class.getClassLoader().getResource("rescuecore2/view/cross.png"));
	...
	private class ShapeViewer extends JComponent {
		...
		class MenuAction extends AbstractAction {
			...
			public MenuAction(ShapeInfo shape) {
				...
				putValue(Action.SMALL_ICON, shape.isVisible() ? TICK : CROSS);
			}
	...
	public static class Line2DShapeInfo extends ShapeInfo {
		...
		public Shape paint(Graphics2D g, ScreenTransform transform) {
			...
			for (Line2D line : lines) {
				...
				if (GeometryTools2D.getDistance(new Point2D(x1, y1), new Point2D(x2, y2)) < 3)
					continue;
