...
public class CommandLayer extends StandardViewLayer {
    ...
    */
    ...
    public void initialise(Config config) {
    	...
    	clearRad=config.getIntValue(REPAIR_RAD_KEY);
    	clearRange=config.getIntValue(REPAIR_DISTANCE_KEY);
    }
    ...
    */
        ...
        synchronized (commands) {
            ...
            for (Command next : commands) {
                if (renderMove && next instanceof AKMove) {
                    Shape shape = renderMove((AKMove)next);
                    result.add(new RenderedObject(world.getEntity(next.getAgentID()), shape));
                }
    ...
    private Shape renderMove(AKMove move) {
    	Human hum = (Human) world.getEntity(move.getAgentID());
    	if(hum instanceof rescuecore2.standard.entities.Civilian)
    		return null;
        g.setColor(getColor(hum));
        ...
        while (it.hasNext()) {
            ...
            if(!it.hasNext()&&(move.getDestinationX()>=0&&move.getDestinationY()>=0))
            	break;
    ...
    if(move.getDestinationX()>=0||move.getDestinationY()>=0){

        int nextX = t.xToScreen(move.getDestinationX());
        int nextY = t.yToScreen(move.getDestinationY());
        g.drawLine(startX, startY, nextX, nextY);
        // Draw an arrow partway along the length
        DrawingTools.drawArrowHeads(startX, startY, nextX, nextY, g);
        startX=t.xToScreen(move.getDestinationX());
        startY=t.yToScreen(move.getDestinationY());
    }
        Ellipse2D.Double shape = new Ellipse2D.Double(startX - SIZE / 2, startY - SIZE / 2, SIZE, SIZE);
		g.draw(shape);
		shape = new Ellipse2D.Double(startX - SIZE / 2 / 4, startY - SIZE / 2 / 4, SIZE / 4, SIZE / 4);
		g.fill(shape);
		return shape;
    }
    private Color getColor(Human h) {
	...
	private void renderClearArea(AKClearArea c) {
		...
		Area area = getClearArea(agent, targetX, targetY, clearRange, clearRad);
        g.setColor(CLEAR_COLOUR);
		drawArea(area);

        Shape shape = new Ellipse2D.Double(t.xToScreen(c.getDestinationX()), t.yToScreen(c.getDestinationY()), 5, 5);
        g.setColor(CLEAR_COLOUR);
        g.fill(shape);
