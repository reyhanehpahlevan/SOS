 ...
 */
public class SOSLogViewer extends JFrame implements ViewListener {
	private static final long serialVersionUID = 1L;

	private static final String VIEWERS_KEY = "log.viewers";

	private SOSFileLogReader log;
	 ...
	 */
	public SOSLogViewer(SOSFileLogReader reader, Config config, String title) throws LogException, InterruptedException {
		super(title);
		try {
			this.log = reader;
			...
			new Thread(new Runnable() {
				...
				@Override
				public void run() {
					System.out.print("Rendering SOS WorldModels");
			...
			totalList.add("Civilian:" + model.getEntitiesOfType(StandardEntityURN.CIVILIAN).size());
...
//		addShowCommandBox(panel);
	...
	private void addIntervalEachCycleSlider(JPanel panel) {
		...
		intervalEachCycleSlider = new JSlider(JSlider.HORIZONTAL, 0, 2000, 1000);
		...
		try {
			...
			if (model != null) {
				...
				if (updatesRecord != null) {
					...
					for (EntityID entityID : updatesRecord.getChangeSet().getChangedEntities())
...
//			processJarFiles(config);
			SOSFileLogReader reader = new SOSFileLogReader(name, Registry.SYSTEM_REGISTRY);
			/* SOSLogViewer viewer = */new SOSLogViewer(reader, config, "SOS Log Namayangar: " + name);
