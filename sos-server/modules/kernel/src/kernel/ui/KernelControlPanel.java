...
import kernel.ui.KernelControlPanel.KernelControlPanelListener.Status;
...
public class KernelControlPanel extends JPanel {
	...
	private static Collection<JButton> controlButtons;
	private static JButton stepButton;
	private static JButton runButton;

	private static volatile boolean running;
	private static volatile boolean step;
	private RunThread runThread;
	private static ArrayList<KernelControlPanelListener> listener = new ArrayList<KernelControlPanelListener>();
	 ...
	 *            The kernel component manager.
	 * @param registry
	 *            The registry to use for new connections.
	 */
	...
	public static boolean isValidForKernelStatusChanging() {
		return stepButton != null;
	}

	public static String stepButtonPressed() {
		// This method should only be called on the event dispatch thread so
		// it's OK to update the GUI.
		...
		synchronized (runLock) {
			if (!running) {
				...
				runLock.notifyAll();
				fireListeners(Status.StepStart);
			}
		}
	...
	private void endStep() {
		...
		fireListeners(Status.StepEnd);
	}

	public static void runButtonPressed() {
		// This method should only be called on the event dispatch thread so
		// it's OK to update the GUI.
		// Do a sanity check just in case.
		if (!SwingUtilities.isEventDispatchThread()) {
		...
		synchronized (runLock) {
			if (running) {
				...
				fireListeners(Status.Stop);
			} else {
				setControlButtonsEnabled(false);
				stepButton.setEnabled(false);
				runButton.setText("Stop");
				fireListeners(Status.Start);
			}
			running = !running;
			runLock.notifyAll();
		}
	...
	public static String startButtonPressed() {
		// This method should only be called on the event dispatch thread so
		// it's OK to update the GUI.
		// Do a sanity check just in case.
		if (!SwingUtilities.isEventDispatchThread()) {
	...
	public static String stopButtonPressed() {
		// This method should only be called on the event dispatch thread so
		// it's OK to update the GUI.
		// Do a sanity check just in case.
		if (!SwingUtilities.isEventDispatchThread()) {
	...
	public static void setControlButtonsEnabled(boolean b) {
		for (JButton next : controlButtons) {
			next.setEnabled(b);
		}
	}

	public static void addListener(KernelControlPanelListener l) {
	...
	private class RunThread extends WorkerThread {
		...
		public boolean work() throws InterruptedException {
			if (shouldStep()) {
				...
				synchronized (runLock) {
					// runLock.wait(config.getIntValue("kernel.agents.think-time"));
					runLock.wait(1000);
				}
				return true;
			}
	...
	private static void fireListeners(Status s) {
		for (KernelControlPanelListener l : listener) {
			l.eventProcess(s);
		}
	}

	public interface KernelControlPanelListener {
		public enum Status {
			StepStart, StepEnd, Start, Stop, Finished
		};
