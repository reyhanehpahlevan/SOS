...
public class SimulatorProxy extends AbstractKernelComponent {
	 ...
	 * @param c
	 *            The connection this simulator is using.
	 */
	 ...
	 *
	 * @param time
	 *            The timestep to get updates for.
	 * @return A ChangeSet representing the updates from this simulator.
	 * @throws InterruptedException
	 *             If this thread is interrupted while waiting for updates.
	 */
	...
	protected void updateReceived(int time, ChangeSet changes) {
		Logger.info(getName()+" took: "+(System.currentTimeMillis()-startTime)+"ms");
		synchronized (updates) {
	...
	private class SimulatorConnectionListener implements ConnectionListener {
		...
		private boolean isIdInUsed(EntityID newId) {
			for (ChangeSet changeSet : updates.values()) {
				if (changeSet.getChangedEntities().contains(newId)
						|| changeSet.getDeletedEntities().contains(newId))
					return true;
			}
			return false;
		}
