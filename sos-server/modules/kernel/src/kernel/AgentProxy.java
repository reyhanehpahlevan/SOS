...
public class AgentProxy extends AbstractKernelComponent {
	 ...
	 *
	 * @param name
	 *            The name of the controlling agent.
	 * @param e
	 *            The entity controlled by the agent.
	 * @param c
	 ...
	 *            The current timestep.
	 * @param visible
	 *            The set of visible changes.
	 * @param heard
	 *            The set of communication messages that the agent heard.
	 */
...
//					if (!(getControlledEntity().getURN().contains("civilian"))&&
//							(!msg.getURN().contains("urn:rescuecore2.standard:message:speak")))
//							Logger.debug("Command received from " + id + "time:" + ((Command) msg).getTime() + " :" + msg.getClass().getSimpleName());
