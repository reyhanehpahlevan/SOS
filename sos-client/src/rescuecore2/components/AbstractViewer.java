...
public abstract class AbstractViewer<T extends WorldModel<? extends Entity>> extends AbstractComponent<T> implements Viewer {
    ...
    */
    ...
    @Override
	public void postConnect(Connection c, int id, Collection<Entity> entities, Config kernelConfig) throws Exception {
        this.viewerID = id;
        lastUpdateTime = 0;
    ...
    protected void handleTimestep(KVTimestep timestep) {
        ...
        lastUpdateTime = time;
        model().merge(changes);
        ((SampleWorldModel)model()).setTime(time);
        }
    ...
    private class ViewerConnectionListener implements ConnectionListener {
        ...
        @Override
		public void messageReceived(Connection c, Message msg) throws Exception {
		...
		private void handleConnectOK(Connection c, KVConnectOK ok) throws Exception {
