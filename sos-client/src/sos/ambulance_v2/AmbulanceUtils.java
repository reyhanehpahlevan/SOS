...
public class AmbulanceUtils {
	...
	public static boolean isValidToDecide(Human hm, SOSLoggerSystem log, AmbulanceTeamAgent agent) {
		...
		ATlogWriter xmlLog = agent.xmlLog;
		String infos = "";
		...
		if (!hm.isPositionDefined()) {
			...
			infos += xmlLog.addTag("Validity", "invalid");
		...
		if (!hm.isHPDefined()) {
			...
			infos += xmlLog.addTag("Validity", "invalid");
			infos += xmlLog.addTag("HP", "unDefined");
			xmlLog.Info(infos);
			return false;
		}
		if (hm.getHP() == 0) {
		...
		if (hm instanceof Civilian) {
			...
			if (hm.getBuriedness() == 0 && hm.getDamage() == 0 && hm.getHP() == 10000) {//TODO if near fire??????
				...
				infos += xmlLog.addTag("Damage", "0");
				infos += xmlLog.addTag("HP", "10000");
				infos += xmlLog.addTag("Buriedness", "0");
				xmlLog.Info(infos);
				return false;
			}
		...
		if (!(isReachableForAT(hm, true))) {
			...
			infos += xmlLog.addTag("Validity", "invalid");
			infos += xmlLog.addTag("Reachability", "false");
			xmlLog.Info(infos);
			return false;
		}
	...
	public static boolean isValidToDecide(VirtualCivilian virtualcivilian, AmbulanceTeam AT, SOSLoggerSystem log, AmbulanceTeamAgent agent) {
		...
		ATlogWriter xmlLog = agent.xmlLog;
		String infos = "";
		infos += xmlLog.addTag("VirtualCivilian", virtualcivilian.toString());

		if (virtualcivilian.isIgnored()) {
			log.trace(virtualcivilian + " is ignored");
			infos += xmlLog.addTag("Validity", "invalid");
			infos += xmlLog.addTag("Ignored", "true");
			xmlLog.Info(infos);
			return false;
		}
		if (virtualcivilian.getDeathTime() + 30 <= AT.model().time()) {
			...
			infos += xmlLog.addTag("Validity", "invalid");
			infos += xmlLog.addTag("DeadTime", "30 cycle before");
			xmlLog.Info(infos);
			return false;
		}
		...
		if ((virtualcivilian.getPosition() instanceof Road) && AT.model().refuges().isEmpty()) {
			...
			infos += xmlLog.addTag("Validity", "invalid");
			infos += xmlLog.addTag("Position", virtualcivilian.getPosition().toString());
			infos += xmlLog.addTag("refuge", "0");
			xmlLog.Info(infos);
			return false;
		}
		...
		if (virtualcivilian.getBuridness() == 0) {
			...
			infos += xmlLog.addTag("Validity", "invalid");
			infos += xmlLog.addTag("Burriedness", "0");
			xmlLog.Info(infos);
			return false;
		}
