...
public abstract class AbstractAmbulanceTeamAgent extends PlatoonAgent<AmbulanceTeam> {
	//	private Human m_loadingInjured = null;
	private Human underMissionTarget = null;

	public Human loadingInjured() {
	...
	public boolean isLoadingInjured() {
		return loadingInjured() != null;
	}

	//	protected void checkLoadingInjured() {
	//		if (loadingInjured() instanceof Human && loadingInjured().isPositionDefined() && loadingInjured().getPosition() != me()) {
	//			// baseLogger.exportln("FUCK! bad loading? what r u talkng about? "+loadingInjured().position());
	//		}
	//	}

	public void setUnderMissionTarget(Human hum) {
		underMissionTarget = hum;
	}
