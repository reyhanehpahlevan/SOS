 ...
 *
 */
public class OldParentCenterMessageSystem implements MessageConstants {
	public Type type;
	...
	public OldParentCenterMessageSystem(SOSAgent<? extends StandardEntity> sosAgent) {
	...
	private void convertAgents() {
		...
		for (Center centre : sosAgent.model().centers()) {
			MessageAgent messageAgent = new MessageAgent(centre, Math.min(getMessageConfig().getCenterSubScribeLimit(), getMessageConfig().radioChannels().size()), index);
...
//			type = Type.WithCenter;
			sosAgent.sosLogger.messageTransmit.info("set channels if centerHear>=4 & platonHear>=2");
...
//			type = Type.NoCenter;
			int middleManNum = estimateNoCenterMessageMiddelManSize();
			sosAgent.sosLogger.messageTransmit.debug("We Use " + middleManNum + " MiddleMan");
			sosAgent.sosLogger.messageTransmit.info("set channels reciver and sender for HumanAgents");
			if (middleManNum > 0) {
				for (int i = 0; i < middleManNum; i++) {
					messageHumans.get(messageHumans.size() - i - 1).setMiddleMan(true);
					for (int j = 0; j < messageConfig.getPlatoonSubScribeLimit(); j++) {
						messageHumans.get(messageHumans.size() - i - 1).addSendChannels(remainedchannels.get(j));
					}
				}
				for (MessageAgent messageHuman : messageHumans) {
					if (!messageHuman.isMiddleMan()) {
						for (int j = 0; j < messageConfig.getPlatoonSubScribeLimit(); j++) {
				...
				for (int j = 0; j < messageConfig.getPlatoonSubScribeLimit(); j++) {
					remainedchannels.remove(j);
				}
				int i = 0;
				while (remainedchannels.size() > 0) {
					if (!messageHumans.get(messageHumans.size() - i - 1).addSubscribeChannels(remainedchannels.remove((int) (Math.random() * remainedchannels.size()))))//for distributing motaadel
						break;
					i = (++i % middleManNum);
				}
			} else {//if(middelManNum==0)
				for (MessageAgent messageHuman : messageHumans) {
					for (int i = 0; i < Math.min(remainedchannels.size(),messageConfig.getPlatoonSubScribeLimit()); i++) {
						messageHuman.addSubscribeChannels(remainedchannels.get(0));
						messageHuman.addSendChannels(remainedchannels.get(0));
					}
				}
				for (int i = 0; i < Math.min(remainedchannels.size(),messageConfig.getPlatoonSubScribeLimit()); i++) {
					remainedchannels.remove(0);
	...
	private int estimateNoCenterMessageMiddelManSize() {
		int platoonHear = getMessageConfig().getPlatoonSubScribeLimit();
		if (messageHumans.size() <= 5)
			return 0;
		if (getMessageConfig().radioChannels().size() > platoonHear + 3 * Math.ceil(platoonHear / 1.5f))// centerHear+centerHear/2+centerHear/2+platoonHear
			return Math.min(platoonHear, 3);
		if (getMessageConfig().radioChannels().size() > platoonHear + 2 * Math.ceil(platoonHear / 1.5f))
			return Math.min(platoonHear, 2);
		return 0;
	}
