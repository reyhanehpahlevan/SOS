...
public class UpdateClusterFireLayer extends SOSAbstractToolsLayer<UpdateClusterFireState> {
	...
	protected Shape render(UpdateClusterFireState entity, Graphics2D g, ScreenTransform transform) {
		...
		for (Building b : entity.allSafe) {
			shape = NamayangarUtils.transformShape(b, transform);
			NamayangarUtils.drawString("safe gard " + b.priority(), g, transform, b);
			g.draw(shape);
		}
		...
		if (entity.lastTarget == null)
			return null;
		g.setColor(Color.GREEN);
		NamayangarUtils.drawString("target", g, transform, entity.lastTarget);
		shape = NamayangarUtils.transformShape(entity.lastTarget, transform);
		g.draw(shape);
		return null;
	}
