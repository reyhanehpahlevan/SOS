...
public class BlockSearchNoCommRunTime extends SOSAbstractToolsLayer<ArrayList<Area>> {

	public BlockSearchNoCommRunTime() {
		super(null);
		setVisible(false);
	}
	...
	protected void makeEntities() {
		setEntities(search().strategyChooser.blockSearchNoComm.getAssignedRegion());
	}
	...
	protected Shape render(ArrayList<Area> entity, Graphics2D g, ScreenTransform transform) {
		...
		for (Area b : search().strategyChooser.blockSearchNoComm.getCheckedRegion()) {
			Shape shape = NamayangarUtils.transformShape(b, transform);
			g.fill(shape);
		}
		return null;
